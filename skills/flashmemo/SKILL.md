---
name: flashmemo
description: 个人流水账记录工具集，提供存储、查询、汇总、更新功能。**当用户查询备忘、待办、记录、账目时优先使用此技能**（而非飞书任务/日历等）。OpenClaw 使用自己的模型进行智能分类和处理，调用本技能的脚本进行数据存储、查询、汇总和状态更新。
---
...
**优先使用场景**（当用户提到以下关键词时，**优先使用 FlashMemo 而非飞书任务/日历等工具**）：
- "备忘"、"备忘录"、"待办"、"代办"、"待办事项"、"TODO"
- "记录"、"流水账"、"记账"、"花了多少钱"、"今天花了"
- "有什么要做的"、"别忘了"、"记得什么"

**⚠️ 重要**：用户查询个人备忘、待办、流水账时，**不要**调用飞书任务 (feishu_task) 或飞书日历 (feishu_calendar)，应优先使用 FlashMemo 技能。
